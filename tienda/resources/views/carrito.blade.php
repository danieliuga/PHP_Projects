...
<body class="font-sans bg-gradient-to-r from-blue-300 to-blue-600 m-0 p-0">
    <div class="mx-auto px-4 py-20">
        ...
        <header class="bg-gradient-to-r from-blue-500 to-blue-400 py-5 px-4 rounded-b-lg text-center -mt-14">
            <h1 class="text-white text-3xl font-bold tracking-wide mb-4">Carrito Shop</h1>
        ...
        <div class="grid grid-cols-3 gap-4 w-full max-w-full mt-10">
            @foreach ($compras as $compra)
                @if ($compra->estado == 'pendiente')
                    <div class="item" style="background-image: url('{{ $compra->vehiculos->imagen }}');">
                        <p class="text-gray-600 flex-grow mb-4">Id: {{ $compra->id }}</p>
                        <p class="text-gray-600 flex-grow mb-4">Nombre: {{ $compra->user->name }}</p>
                        <p class="text-gray-600 flex-grow mb-4">Modelo: {{ $compra->vehiculos->modelo }}</p>
                        <p class="text-gray-600 flex-grow mb-4">Precio: {{ $compra->vehiculos->precio }}</p>
                        <div class="flex justify-center mt-4 space-x-4">
                            <a href="/comprar/vehiculo/{{ $compra->vehiculos->modelo }}"
                                class="inline-block px-3 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 z-10">Comprar</a>
                            <a href="/carrito/eliminar/{{ $compra->id }}"
                                class="inline-block px-3 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 z-10">Eliminar</a>
                        </div>
                    </div>
                @endif
            @endforeach
        ...
        <div class="text-center mt-5 mb-3 z-10">
            <form action="{{ route('comprar.todo') }}" method="POST">
                @csrf
                <button class="px-4 py-2 bg-blue-500 text-white rounded-md text-base hover:bg-blue-600" type="submit">Comprar todo</button>
            </form>
        </div>
