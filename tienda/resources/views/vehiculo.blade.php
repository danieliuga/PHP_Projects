...
{{-- 
    - volve a la ... ponerlo abajo del todo (carrito,)  
    - añadir plugins
--}}
...
<body class="font-sans bg-gradient-to-r from-blue-300 to-blue-600 m-0 p-0">
    <div class="mx-auto px-4 py-20">
        ...
        <header class="bg-gradient-to-r from-blue-500 to-blue-400 py-5 px-4 rounded-b-lg text-center -mt-14">
            <h1 class="text-white text-3xl font-bold tracking-wide mb-4">Supercars Shop</h1>
        ...
        <div class="grid grid-cols-3 gap-4 w-full max-w-full mt-10 mb-10">
            @foreach ($vehiculos as $vehiculo)
                <div class="car-listing">
                    <img src="{{ asset($vehiculo->imagen) }}" alt="{{ $vehiculo->modelo }}"
                        class="w-full max-w-full h-48 object-cover rounded-md" />
                    <h3 class="mt-4 text-gray-900 mb-4">{{ $vehiculo->modelo }}</h3>
                    <p class="text-gray-600 flex-grow mb-4">Precio: {{ number_format($vehiculo->precio, 2) }} €</p>
                    <p class="text-gray-600 flex-grow mb-4">Stock: {{ number_format($vehiculo->stock) }}</p>
                    <p class="text-gray-600 flex-grow mb-4">Descripción: {{ $vehiculo->descripcion }}</p>
                    <a href="/carrito/añadir/{{ $vehiculo->modelo }}"
                        ...
                        @auth
                            @foreach (auth()->user()->cuenta_bancarias as $cuenta_bancaria)
                                @if ($cuenta_bancaria->dinero >= $vehiculo->precio)
                                    Añadir al carrito
                                @else
                                    Dinero insuficiente
                                @endif
