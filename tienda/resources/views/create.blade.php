...
<body class="font-sans bg-gradient-to-r from-blue-300 to-blue-600 m-0 p-0">
    <div class="mx-auto px-4 py-20">
        ...
        <header class="bg-gradient-to-r from-blue-500 to-blue-400 py-5 px-4 rounded-b-lg text-center -mt-14">
            <h1 class="text-white text-3xl font-bold tracking-wide mb-4">Crear Vehiculo</h1>
        ...
        <form action="{{ route('modificar.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md mb-6 mt-6">
            @csrf
            <div class="mb-6">
                <label for="modelo" class="block font-bold mb-1">Modelo:</label>
                <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" id="modelo" name="modelo">
            </div>
            <div class="mb-6">
                <label for="precio" class="block font-bold mb-1">Precio:</label>
                <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" id="precio" name="precio">
            </div>
            <div class="mb-6">
                <label for="stock" class="block font-bold mb-1">Stock:</label>
                <input type="text" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" id="stock" name="stock">
            </div>
            <div class="mb-6">
                <label for="descripcion" class="block font-bold mb-1">Descripción:</label>
                <textarea class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500 resize-none" id="descripcion" name="descripcion"></textarea>
            </div>
            <div class="mb-6">
                <label for="imagen" class="block font-bold mb-1">Imagen:</label>
                <input type="file" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" id="imagen" name="imagen">
            </div>
            <div class="mb-6">
                <label for="tipo_id" class="block font-bold mb-1">Tipo de Vehículo:</label>
                <select class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" id="tipo_id" name="tipo_id">
                    @foreach ($tipos as $tipo)
                        <option value="{{ $tipo->id }}">{{ $tipo->nombre }}</option>
                    @endforeach
                </select>
            </div>
            <button type="submit" class="px-6 py-2 rounded-md bg-blue-500 text-white hover:bg-blue-600 transition-colors duration-300">Crear Vehículo</button>
        </form>
