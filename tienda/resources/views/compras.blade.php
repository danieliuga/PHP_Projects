...
<body class="font-sans bg-gradient-to-r from-blue-300 to-blue-600 m-0 p-0">
    <div class="mx-auto px-4 py-20">
        ...
        <header class="bg-gradient-to-r from-blue-500 to-blue-400 py-5 px-4 rounded-b-lg text-center -mt-14">
            <h1 class="text-white text-3xl font-bold tracking-wide mb-4">Lista de Compras</h1>
        ...
        <h3 class="text-2xl text-white mt-4">Compras</h3>
        @if (count($compras) > 0)
            <div class="mt-6">
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-blue-700">
                            <th class="border border-gray-300 py-2 px-4 text-white">ID</th>
                            <th class="border border-gray-300 py-2 px-4 text-white">Usuario</th>
                            <th class="border border-gray-300 py-2 px-4 text-white">Vehículo</th>
                            <th class="border border-gray-300 py-2 px-4 text-white">Tipo</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php $count = 0 @endphp
                        @foreach ($compras as $compra)
                            @if ($compra->estado == 'comprado')
                                @php $count++ @endphp
                                <tr class="{{ $count % 2 === 0 ? 'bg-blue-500' : 'bg-blue-400' }}">
                                    <!-- Alternamos los estilos según el valor del contador -->
                                    <td class="border border-gray-300 py-2 px-4 text-white">{{ $compra->id }}</td>
                                    <td class="border border-gray-300 py-2 px-4 text-white">{{ $compra->user->name }}
                                    </td>
                                    <td class="border border-gray-300 py-2 px-4 text-white">
                                        {{ $compra->vehiculos->modelo }}</td>
                                    <td class="border border-gray-300 py-2 px-4 text-white">{{ $compra->tipo->nombre }}
                                    </td>
                                </tr>
                            @endif
                        @endforeach
        ...
        @else
            <p class="mt-20">No hay compras.</p>
        @endif



        <h3 class="text-2xl text-white mt-4">Ventas</h3>
        @if (count($ventas) > 0)
            <div class="mt-6 mb-6">
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-blue-700">
                            <th class="border border-gray-300 py-2 px-4 text-white">ID</th>
                            <th class="border border-gray-300 py-2 px-4 text-white">Usuario</th>
                            <th class="border border-gray-300 py-2 px-4 text-white">Accesorio</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php $count = 0 @endphp
                        @foreach ($ventas as $venta)
                            @php $count++ @endphp
                            <tr class="{{ $count % 2 === 0 ? 'bg-blue-500' : 'bg-blue-400' }}">
                                <td class="border border-gray-300 py-2 px-4 text-white">{{ $venta->id }}</td>
                                <td class="border border-gray-300 py-2 px-4 text-white">{{ $venta->user->name }}</td>
                                <td class="border border-gray-300 py-2 px-4 text-white">
                                    {{ $venta->accesorios->modelo }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
