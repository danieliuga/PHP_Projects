...
<body class="font-sans bg-gradient-to-r from-blue-300 to-blue-600 m-0 p-0">
    <div class="mx-auto px-4 py-20">
        ...
        <header class="bg-gradient-to-r from-blue-500 to-blue-400 py-5 px-4 rounded-b-lg text-center -mt-14">
            <h1 class="text-white text-3xl font-bold tracking-wide mb-4">Users</h1>
        ...
        @if (count($users) > 0)
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-blue-700">
                            ...
                            <th class="border border-gray-300 py-2 px-4 text-white">Nombre</th>
                            <th class="border border-gray-300 py-2 px-4 text-white">Email</th>
                            <th class="border border-gray-300 py-2 px-4 text-white">Money</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php $count = 0 @endphp
                        @foreach ($users as $user)
                            ...
                            <tr class="{{ $count % 2 === 0 ? 'bg-blue-500' : 'bg-blue-400' }}">
                                <td class="border border-gray-300 py-2 px-4 text-white">{{ $user->id }}</td>
                                <td class="border border-gray-300 py-2 px-4 text-white">{{ $user->name }}</td>
                                <td class="border border-gray-300 py-2 px-4 text-white">{{ $user->email }}</td>
                                <td class="border border-gray-300 py-2 px-4 text-white">
                                    @if ($user->cuenta_bancarias->count() > 0)
                                        @foreach ($user->cuenta_bancarias as $cuenta_bancaria)
                                            {{ $cuenta_bancaria->dinero }}
                                        @endforeach
                                    @else
                                        0.00
                                    @endif
            ...
            @else
                <p class="mt-20">No hay usuarios.</p>
        @endif


        <div class="bg-black text-white px-2 py-1 rounded-md text-center mt-6">
